...
themes=("autogenerated-from-wallpaper" "3024-dark" "3024-light" "ashes-dark" "ashes-light" "base16-3024" "base16-apathy" "base16-ashes"
...
if [ "$selected" = "autogenerated-from-wallpaper" ]; then
    ...
    sed -i '/^wallust/c\wallust run "$SELECTED_WALL"' "$wal_dir"
    echo "Theme set to autogenerate from wallpaper"
else
    # For regular themes, use wallust theme command
    sed -i '/^wallust/c\wallust theme '"$selected" "$wal_dir"
